 ...
 *
 */
public class MetaPrint2D_DS_Generator implements IGenerator<IAtomContainer> {

    private static final Logger logger = Logger.getLogger(MetaPrint2D_DS_Generator.class);

    public MetaPrint2D_DS_Generator() {
    ...
    public IRenderingElement generate( IAtomContainer ac,
        ...
        if (atomMap.getValue().size()<=0){
        	logger.error("A MetaPrint2D_DS_Generator is used, but AtomMap was empty.");
        	return group;
        }
        ...
        for(int i = 0;i<ac.getAtomCount();i++) {  //Loop over all atoms
            for (Integer ii : atomMap.getValue().keySet()){   //Loop over list of atom indices with a result
                if (ii.intValue()==i){
                    ...
                    Color drawColor=MetaPrint2DHelper.getMetaPrint2DColor(resValue);
                    if (drawColor!=null)
